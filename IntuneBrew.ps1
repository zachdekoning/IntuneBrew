...
function Get-AppFile($url, $fileName, $expectedHash) {
    ...
    try {
        ...
        try {
            ...
            # First check if the URL contains a filename hint
            if ($url -match '\/([^\/\?]+\.(dmg|pkg))[\?\&]') {
                $finalFileName = $matches[1]
                Write-Host "Extracted filename from URL: $finalFileName" -ForegroundColor Gray
            }
            # If URL doesn't have a clear filename, try to get it from headers
            else {
                try {
                    # Make a GET request to get headers (HEAD might not work with all servers)
                    $request = [System.Net.HttpWebRequest]::Create($url)
                    $request.Method = "GET"
                    $request.AllowAutoRedirect = $true
                    $request.Timeout = 30000
                    # Only get headers, don't download the full content
                    ...
                    if ($contentDisposition -and $contentDisposition -match 'filename=["]*([^"]+)["]*') {
                        $extractedFileName = $matches[1].Trim('"').Trim("'")
                        Write-Host "Extracted filename from headers: $extractedFileName" -ForegroundColor Gray
                        $finalFileName = $extractedFileName
                    }
                    # If no Content-Disposition, try to get from the final URL
                    elseif ($response.ResponseUri.Segments.Length -gt 0) {
                        $lastSegment = $response.ResponseUri.Segments[-1]
                        if ($lastSegment -match '(.*\.(dmg|pkg))') {
                            $finalFileName = $lastSegment
                            Write-Host "Extracted filename from response URL: $finalFileName" -ForegroundColor Gray
                        }
                    }
                    $response.Close()
                }
                catch {
                    Write-Host "Could not extract filename from headers, using default: $sanitizedFileName" -ForegroundColor Yellow
                }
            }
            
            # If we still have an invalid filename (with ? or other invalid chars), use a default
            if ($finalFileName -match '[\/\\\:\*\?\"\<\>\|]') {
                $extension = if ($url -match '\.(dmg|pkg)') { $matches[1] } else { "dmg" }
                $finalFileName = "raycast.$extension"
                Write-Host "Using default filename due to invalid characters: $finalFileName" -ForegroundColor Yellow
            }
            
            # Update the output path with the final filename
            $outputPath = Join-Path $PWD $finalFileName
            Write-Host "Final download path: $outputPath" -ForegroundColor Gray
    ...
    if ($actualHashNormalized -eq $expectedHashNormalized) {
        ...
        Write-Host "   • Expected hash: $expectedHashNormalized" -ForegroundColor Yellow
        Write-Host "   • Actual hash: $actualHashNormalized" -ForegroundColor Yellow
        Write-Host "`nThis could happen if:"
        ...
        if ($continueAnyway.ToLower() -eq 'y') {
            ...
            Write-Host "`nAborting operation due to hash mismatch." -ForegroundColor Red
            Remove-Item $outputPath -Force
            Write-Host "`n"
...
# Helper function to format TimeSpan for the update summary
function Format-TimeSpanForSummary {
    param (
    ...
    if ($TimeSpan.TotalMinutes -ge 1) {
        ...
        if ($roundedMinutes -eq 0 -and $TimeSpan.TotalMinutes > 0) {
            ...
            return "a moment" # Fallback for very small fractions of a minute
        }
        return "$roundedMinutes minute$(if ($roundedMinutes -ne 1) {'s'} else {''})"
    }
    elseif ($TimeSpan.TotalSeconds -ge 1) {
        ...
        return "a moment" # For durations less than 1 second
    }
}
...
foreach ($app in $appsToUpload) {
    ...
    $fileName = [System.IO.Path]::GetFileNameWithoutExtension($appFilePath)
    
    # Check if we need to rename the file (if it's named "download" or missing extension)
    if ($fileName -eq "download" -or $fileExtension -eq "" -or $fileExtension -notin @('.dmg', '.pkg')) {
        # Determine the expected extension from the app info
        $expectedExtension = if ($appInfo.fileName -match '\.(dmg|pkg)$') {
            ".$($matches[1])"
        }
        else {
            # Default to .dmg if we can't determine
            ".dmg"
        }
        ...
        # Rename the file
        Write-Host "Renaming file to use app name: $properFileName" -ForegroundColor Yellow
        Move-Item -Path $appFilePath -Destination $newFilePath -Force
    ...
    # Determine app type based on the actual downloaded file extension
    $fileExtension = [System.IO.Path]::GetExtension($appFilePath).ToLower()
    $appType = if ($fileExtension -eq '.dmg') {
        "macOSDmgApp"
    }
    elseif ($fileExtension -eq '.pkg') {
        "macOSPkgApp"
    }
    else {
    ...
    $endTime = Get-Date # Record end time for this app's update
...
if ($updateSummaries.Count -gt 0) {
    Write-Host "`n📊 Summary:" -ForegroundColor Cyan
    Write-Host "---------------------------------------------------" -ForegroundColor Yellow
    foreach ($summaryEntry in $updateSummaries) {
        $durationFormatted = Format-TimeSpanForSummary $summaryEntry.Duration
        if ($summaryEntry.OldVersion -eq "Newly installed") {
            Write-Host "$($summaryEntry.AppName) (version $($summaryEntry.NewVersion)) was uploaded in $durationFormatted."
        }
        else {
            Write-Host "$($summaryEntry.AppName) was updated from $($summaryEntry.OldVersion) to $($summaryEntry.NewVersion) in $durationFormatted."
        }
    }
    Write-Host "---------------------------------------------------" -ForegroundColor Yellow
}
